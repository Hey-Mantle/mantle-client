...
/**
 * Commission rules for an affiliate program
 */
interface AffiliateProgramRules {
  /** Percentage commission on referred sales */
  percentCommission?: number;
  /** Duration of commission eligibility in months */
  durationMonths?: number;
  /** Fixed amount paid per referred install */
...
/**
 * An affiliate program that customers can join to earn commissions
 */
interface AffiliateProgram {
  ...
  name: string;
  /** Commission rules for the program */
  rules: AffiliateProgramRules;
  /** Program branding (logo, description, etc.) */
  branding: Record<string, any> | null;
  /** URL to the program's terms and conditions */
  termsUrl: string | null;
  /** Whether affiliates must accept terms before joining */
  requireTermsAcceptance: boolean;
  /** Whether affiliates must be approved before joining */
  requireApprovalToJoin: boolean;
  /** Fields required when joining the program */
  requiredAffiliateFields: Record<string, any> | null;
  /** Whether affiliates can access a dashboard for this program */
  allowAffiliateDashboardAccess: boolean;
  /** Custom base URL pattern for affiliate referral links */
...
/**
 * An affiliate's membership in a program
 */
interface AffiliateMembership {
  ...
  handle: string;
  /** The affiliate's referral link, null if pending approval or link attribution not supported */
  affiliateLink: string | null;
  /** Effective commission rules for this membership */
  rules: AffiliateProgramRules;
  /** The date the affiliate joined the program */
  joinedAt: string;
  /** The date the affiliate agreed to the program terms */
  agreedToTermsAt: string | null;
  /** The date the affiliate requested approval */
  approvalRequestedAt: string | null;
}

/**
 * An affiliate who can refer customers and earn commissions
 */
interface Affiliate {
  ...
  email: string;
  /** The date the affiliate was created */
  createdAt: string;
  /** The affiliate's membership in the program, null if not enrolled */
  membership: AffiliateMembership | null;
}

/**
 * A confirmed referral attributed to an affiliate
 */
...
/**
 * A referral attribution request submitted by an affiliate
 */
interface AffiliateReferralRequest {
  /** The ID of the referral request */
  id: string;
  /** The name of the referred customer */
  customerName: string | null;
  ...
  /** The date of the referral */
  date: string | null;
  /** The status of the referral request */
...
/**
 * Performance metrics for an affiliate
 */
interface AffiliateMetrics {
  /** The number of confirmed referrals */
  referralCount: number;
  /** Total commissions earned (paid + outstanding) */
  totalCommissions: number;
  /** Commissions that have been paid out */
  commissionsPaid: number;
  /** Commissions that are pending payout */
  outstandingCommissions: number;
  /** Total sales generated by referrals */
  totalSales: number;
}

/**
 * Response from the getAffiliateReferrals method
 */
interface ListAffiliateReferralsResponse {
  /** The list of referrals */
  referrals: AffiliateReferral[];
  /** The total number of referrals */
  total: number;
  /** Whether there are more referrals after the current page */
  hasNextPage: boolean;
  /** Whether there are referrals before the current page */
  hasPreviousPage: boolean;
}

/**
 * Response from the getAffiliateReferralRequests method
 */
interface ListAffiliateReferralRequestsResponse {
  /** The list of referral requests */
  referral_requests: AffiliateReferralRequest[];
  /** The total number of referral requests */
  total: number;
  /** Whether there are more requests after the current page */
  hasNextPage: boolean;
  /** Whether there are requests before the current page */
  hasPreviousPage: boolean;
}
...
class MantleClient {
   ...
   */
  async getAffiliateProgram(params?: {
    customerId?: string;
  }): Promise<AffiliateProgram | MantleError> {
    const response = await this.mantleRequest<{
      affiliate_program: AffiliateProgram;
    }>({
      path: "affiliate_program",
      ...(params?.customerId ? { body: { customerId: params.customerId } } : {}),
    });
  ...
  async getAffiliate(params?: {
    customerId?: string;
  }): Promise<Affiliate | null | MantleError> {
    const response = await this.mantleRequest<{ affiliate: Affiliate | null }>({
      path: "affiliate",
      ...(params?.customerId ? { body: { customerId: params.customerId } } : {}),
    });
   ...
   */
  async enrollAffiliate(params?: {
    name?: string;
    email?: string;
    agreedToTerms?: boolean;
    customerId?: string;
  }): Promise<Affiliate | MantleError> {
    const response = await this.mantleRequest<{ affiliate: Affiliate }>({
      path: "affiliate",
      method: "POST",
      body: params,
    });
   ...
   */
  async submitReferralRequest(params: {
    shopDomain?: string;
    customerName?: string;
    notes?: string;
    date?: string;
    ...
    const response = await this.mantleRequest<{
      referral_request: AffiliateReferralRequest;
    }>({
  ...
  async getAffiliateReferrals(params?: {
    ...
    return await this.mantleRequest<ListAffiliateReferralsResponse>({
      path: "affiliate/referrals",
      body: {
        page: params?.page ?? 0,
   ...
   */
  async getAffiliateReferralRequests(params?: {
    page?: number;
    limit?: number;
    customerId?: string;
  }): Promise<ListAffiliateReferralRequestsResponse | MantleError> {
    return await this.mantleRequest<ListAffiliateReferralRequestsResponse>({
   ...
   */
  async getAffiliateMetrics(params?: {
    customerId?: string;
  }): Promise<AffiliateMetrics | MantleError> {
    const response = await this.mantleRequest<{ metrics: AffiliateMetrics }>({
